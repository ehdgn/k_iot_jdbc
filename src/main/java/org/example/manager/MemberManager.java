...
public class MemberManager {
    ...
    private void updateMember(Scanner sc, MemberDao dao) throws SQLException {
        System.out.println("수정 할 사용자 ID 입력: ");
        int id = Integer.parseInt(sc.nextLine());
        ...
        System.out.printf("사용자의 새 이름 (변경하지 않으려면 Enter)");
        String newName = sc.nextLine();

        System.out.printf("사용자의 새 이메일 (변경하지 않으려면 Enter)");
        ...
        if(newName.isEmpty() && newEmail.isEmpty()) {
            System.out.println("변경 값이 없습니다. (수정불가)");
            return;
        }
